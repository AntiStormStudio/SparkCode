 ...
 *
 * Inbound messages from spark-ai.top are injected into the REPL via queuedCommands.
 */
  ...
  useEffect(() => {
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      if (consecutiveFailuresRef.current >= MAX_CONSECUTIVE_INIT_FAILURES) {
        ...
        const fuseHint = '连续失败次数过多，已禁用 · 重启后重试';
      ...
      void (async () => {
        try {
          ...
          // Assistant mode: perpetual bridge session — spark-ai.top shows one
          ...
          // When a user message arrives from spark-ai.top, inject it into the REPL.
          ...
          const handle_0 = await initReplBridge({
            ...
            onSetPermissionMode(mode) {
              ...
              //     unchanged (3-way invariant violation per src/SPARK.md)
              ...
              if (mode === 'bypassPermissions') {
                if (isBypassPermissionsModeDisabled()) {
                  return {
                    ok: false,
                    error: '无法将权限模式设为 bypassPermissions：该模式已被设置或配置禁用'
                  };
                }
                if (!store.getState().toolPermissionContext.isBypassPermissionsModeAvailable) {
                  return {
                    ok: false,
                    error: '无法将权限模式设为 bypassPermissions：当前会话并非通过 --dangerously-skip-permissions 启动'
                  };
                }
              }
              if (feature('TRANSCRIPT_CLASSIFIER') && mode === 'auto' && !isAutoModeGateEnabled()) {
                ...
                return {
                  ok: false,
                  error: reason ? `无法将权限模式设为 auto：${getAutoModeUnavailableNotification(reason)}` : '无法将权限模式设为 auto'
                };
              }
          ...
          if (!handle_0) {
            ...
            setAppState(prev_13 => ({
              ...prev_13,
              replBridgeError: prev_13.replBridgeError ?? '请查看调试日志了解详情'
            }));
            failureTimeoutRef.current = setTimeout(() => {
              if (cancelled) return;
              failureTimeoutRef.current = undefined;
          ...
          if (outboundOnly) {
            ...
            if (cancelled) return;
            setMessages(prev_18 => [...prev_18, createBridgeStatusMessage(url, upgradeNudge ? '请将 Claude 移动应用升级到最新版本，以查看你的远程控制会话。' : undefined)]);
