    ...
    t2 = () => {
      ...
      if (failedClaudeAiClients.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="error">{failedClaudeAiClients.length} 个 spark-ai.top 连接器不可用</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
      ...
      if (needsAuthClaudeAiServers.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="warning">{needsAuthClaudeAiServers.length} 个 spark-ai.top 连接器需要认证</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
    };
