...
export type EnvLessBridgeConfig = {
  ...
  min_version: string
  // When true, tell users their spark-ai.top app may be too old to see v2
...
export async function checkEnvLessBridgeMinVersion(): Promise<string | null> {
  ...
  if (cfg.min_version && lt(MACRO.VERSION, cfg.min_version)) {
    return `你当前的 Spark Code 版本（${MACRO.VERSION}）过低，无法使用远程控制。\n需要 ${cfg.min_version} 或更高版本。请运行 \`sparkc update\` 进行更新。`
  }
  return null
}

/**
 * Whether to nudge users toward upgrading their spark-ai.top app when a
