    ...
    case "advisor_result":
      {
        ...
        if ($[24] !== block.content.text || $[25] !== verbose) {
          t1 = verbose ? <Text dimColor={true}>{block.content.text}</Text> : <Text dimColor={true}>{figures.tick} Advisor 已审阅对话，并会应用反馈 <CtrlOToExpand /></Text>;
    ...
    case "advisor_redacted_result":
      {
        ...
        if ($[27] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <Text dimColor={true}>{figures.tick} Advisor 已审阅对话，并会应用反馈</Text>;
          $[27] = t1;
        } else {
          t1 = $[27];
        }
