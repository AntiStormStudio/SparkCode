...
export function PlanApprovalResponseDisplay(t0) {
  ...
  if (response.approved) {
    ...
    if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
      t2 = <Box marginTop={1}><Text>现在可以继续实现。计划模式限制已解除。</Text></Box>;
      $[2] = t2;
    } else {
      t2 = $[2];
    }
  ...
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box marginTop={1}><Text dimColor={true}>请根据反馈修改计划，然后再次调用 ExitPlanMode。</Text></Box>;
    $[9] = t3;
  } else {
    t3 = $[9];
  }
...
function getPlanApprovalSummary(content: string): string | null {
  ...
  if (response) {
    if (response.approved) {
      return '[计划已批准] 现在可以继续实现';
    } else {
      return `[计划已拒绝] ${response.feedback || '请修改计划'}`;
    }
  }
  return null;
}
