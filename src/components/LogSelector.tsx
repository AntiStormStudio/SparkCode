...
export function LogSelector(t0) {
  ...
  if ($[166] !== columns || $[167] !== displayedLogs.length || $[168] !== effectiveTagIndex || $[169] !== focusedIndex || $[170] !== hasTags || $[171] !== showAllProjects || $[172] !== tagTabs || $[173] !== viewMode || $[174] !== visibleCount) {
    t60 = hasTags ? <TagTabs tabs={tagTabs} selectedIndex={effectiveTagIndex} availableWidth={columns} showAllProjects={showAllProjects} /> : <Box flexShrink={0}><Text bold={true} color="suggestion">恢复会话{viewMode === "list" && displayedLogs.length > visibleCount && <Text dimColor={true}>{" "}（第 {focusedIndex} / {displayedLogs.length} 个）</Text>}</Text></Box>;
    $[166] = columns;
  ...
  if ($[187] !== agenticSearchState.results || $[188] !== agenticSearchState.status) {
    t66 = agenticSearchState.status === "results" && agenticSearchState.results.length > 0 && <Box paddingLeft={1} marginBottom={1} flexShrink={0}><Text dimColor={true} italic={true}>Spark 找到了这些结果：</Text></Box>;
  ...
  if ($[190] !== agenticSearchState.results || $[191] !== agenticSearchState.status || $[192] !== filteredLogs) {
    t67 = agenticSearchState.status === "results" && agenticSearchState.results.length === 0 && filteredLogs.length === 0 && <Box paddingLeft={1} marginBottom={1} flexShrink={0}><Text dimColor={true} italic={true}>没有找到匹配的会话。</Text></Box>;
  ...
  if ($[194] !== agenticSearchState.status || $[195] !== filteredLogs) {
    t68 = agenticSearchState.status === "error" && filteredLogs.length === 0 && <Box paddingLeft={1} marginBottom={1} flexShrink={0}><Text dimColor={true} italic={true}>没有找到匹配的会话。</Text></Box>;
  ...
  if ($[222] !== agenticSearchState.status || $[223] !== currentBranch || $[224] !== exitState.keyName || $[225] !== exitState.pending || $[226] !== getExpandCollapseHint || $[227] !== hasMultipleWorktrees || $[228] !== isAgenticSearchOptionFocused || $[229] !== isSearching || $[230] !== onToggleAllProjects || $[231] !== showAllProjects || $[232] !== showAllWorktrees || $[233] !== viewMode) {
    t71 = <Box paddingLeft={2}>{exitState.pending ? <Text dimColor={true}>再次按 {exitState.keyName} 退出</Text> : viewMode === "rename" ? <Text dimColor={true}><Byline><KeyboardShortcutHint shortcut="Enter" action="保存" /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="取消" /></Byline></Text> : agenticSearchState.status === "searching" ? <Text dimColor={true}><Byline><Text>使用 Spark 深度搜索中…</Text><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="取消" /></Byline></Text> : isAgenticSearchOptionFocused ? <Text dimColor={true}><Byline><KeyboardShortcutHint shortcut="Enter" action="搜索" /><KeyboardShortcutHint shortcut={"\u2193"} action="跳过" /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="取消" /></Byline></Text> : viewMode === "search" ? <Text dimColor={true}><Byline><Text>{isSearching && false ? "搜索中…" : "输入搜索"}</Text><KeyboardShortcutHint shortcut="Enter" action="选择" /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="清除" /></Byline></Text> : <Text dimColor={true}><Byline>{onToggleAllProjects && <KeyboardShortcutHint shortcut="Ctrl+A" action={`显示${showAllProjects ? "当前目录" : "所有项目"}`} />}{currentBranch && <KeyboardShortcutHint shortcut="Ctrl+B" action="切换分支" />}{hasMultipleWorktrees && <KeyboardShortcutHint shortcut="Ctrl+W" action={`显示${showAllWorktrees ? "当前 worktree" : "所有 worktree"}`} />}<KeyboardShortcutHint shortcut="Ctrl+V" action="预览" /><KeyboardShortcutHint shortcut="Ctrl+R" action="重命名" /><Text>输入搜索</Text><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="取消" />{getExpandCollapseHint() && <Text>{getExpandCollapseHint()}</Text>}</Byline></Text>}</Box>;
