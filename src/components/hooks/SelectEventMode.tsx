...
export function SelectEventMode(t0) {
  ...
  if ($[2] !== restrictedByPolicy) {
    t2 = restrictedByPolicy && <Box flexDirection="column"><Text color="suggestion">{figures.info} Hooks 受策略限制</Text><Text dimColor={true}>仅允许托管设置中的 Hooks 运行。来自 ~/.claude/settings.json、.claude/settings.json 和 .claude/settings.local.json 的用户自定义 Hooks 会被阻止。</Text></Box>;
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column"><Text dimColor={true}>{figures.info} 此菜单为只读。要添加或修改 Hooks，请直接编辑 settings.json，或让 Claude 协助。{" "}<Link url="https://code.claude.com/docs/en/hooks">了解更多</Link></Text></Box>;
    $[4] = t3;
  } else {
    t3 = $[4];
  }
