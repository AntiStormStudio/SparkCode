...
export function formatReviewStageCounts(stage: ReviewStage | undefined, found: number, verified: number, refuted: number): string {
  ...
  if (!stage) return `${found} 个发现 · ${verified} 个已验证`;
  ...
  if (stage === 'verifying') {
    const parts = [`${found} 个发现`, `${verified} 个已验证`];
    if (refuted > 0) parts.push(`${refuted} 个已排除`);
    return parts.join(' · ');
  }
  // stage === 'finding'
  return found > 0 ? `${found} 个发现` : '查找中';
}
...
function ReviewRainbowLine(t0) {
  ...
  if ($[2] !== found || $[3] !== p || $[4] !== refuted || $[5] !== verified) {
    t1 = !p ? "设置中" : formatReviewStageCounts(p.stage, found, verified, refuted);
    $[2] = found;
    $[3] = p;
    $[4] = refuted;
    $[5] = verified;
