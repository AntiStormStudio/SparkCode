  ...
  if (name === ASK_USER_QUESTION_TOOL_NAME && 'questions' in input) {
    ...
    if (Array.isArray(qs) && qs[0] && typeof qs[0] === 'object') {
      ...
      if (q) {
        ...
        return `在浏览器回答：${truncateToWidth(oneLine, 50)}`;
      }
    }
  }
...
function UltraplanSessionDetail(t0) {
  ...
  const statusText = running ? phase ? PHASE_LABEL[phase] : "运行中" : session.status === "completed" ? "已完成" : session.status;
  ...
  if (confirmingStop) {
    ...
    if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Text dimColor={true}>这将终止网页版 Spark Code 会话。</Text>;
      $[12] = t7;
    } else {
      t7 = $[12];
    }
...
function StagePipeline(t0) {
  ...
  let t2;
  if ($[2] !== inSetup) {
    t2 = inSetup ? <Text color="background">设置</Text> : <Text dimColor={true}>设置</Text>;
    $[2] = inSetup;
    $[3] = t2;
  } else {
    t2 = $[3];
...
function ReviewSessionDetail(t0) {
  ...
  const sessionUrl = t2;
  const statusLabel = completed ? "可查看" : running ? "运行中" : session.status;
  if (confirmingStop) {
    let t3;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
      t4 = <Text dimColor={true}>这会归档远程会话并停止本地跟踪。评审不会完成，目前已有的发现也会被丢弃。</Text>;
      $[5] = t4;
    } else {
      t4 = $[5];
    }
...
export function RemoteSessionDetailDialog({
  ...
  const displayStatusText = displayStatus === 'running' ? '运行中' : displayStatus === 'starting' ? '启动中' : displayStatus === 'completed' ? '已完成' : displayStatus;
  return <Box flexDirection="column" tabIndex={0} autoFocus onKeyDown={handleKeyDown}>
      <Dialog title="远程会话详情" onCancel={handleClose} color="background" inputGuide={exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline>
        ...
        {session.log.length > 0 && <Box flexDirection="column" marginTop={1}>
            ...
            <Box marginTop={1}>
              <Text dimColor italic>
                显示最近 {lastMessages.length} 条，共 {session.log.length}{' '}
                条消息
              </Text>
            </Box>
          </Box>}
        ...
        {teleportError && <Box marginTop={1}>
            <Text color="error">Teleport 失败：{teleportError}</Text>
          </Box>}

        {/* Teleporting status */}
        {isTeleporting && <Text color="background">正在 Teleport 到会话…</Text>}
      </Dialog>
    </Box>;
}
