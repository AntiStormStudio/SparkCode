...
export function formatToolUseSummary(name: string, input: unknown): string {
  ...
  if (name === EXIT_PLAN_MODE_V2_TOOL_NAME) {
    return 'Review the plan in Spark Code on the web';
  }
...
function UltraplanSessionDetail(t0) {
  ...
  if (confirmingStop) {
    ...
    if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Text dimColor={true}>This will terminate the Spark Code on the web session.</Text>;
      $[12] = t7;
    } else {
      t7 = $[12];
    }
  ...
  if ($[47] === Symbol.for("react.memo_cache_sentinel")) {
    t19 = {
      label: "Review in Spark Code on the web",
      value: "open" as const
    };
    $[47] = t19;
  } else {
...
function ReviewSessionDetail(t0) {
  ...
  if ($[11] !== completed || $[12] !== onKill || $[13] !== running) {
    t3 = completed ? [{
      label: "Open in Spark Code on the web",
      value: "open"
    }, {
      label: "Dismiss",
      value: "dismiss"
...
export function RemoteSessionDetailDialog({
  ...
  return <Box flexDirection="column" tabIndex={0} autoFocus onKeyDown={handleKeyDown}>
      <Dialog title="Remote session details" onCancel={handleClose} color="background" inputGuide={exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline>
