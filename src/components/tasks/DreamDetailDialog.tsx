...
export function DreamDetailDialog(t0) {
  ...
  if ($[8] !== elapsedTime || $[9] !== handleKeyDown || $[10] !== onBack || $[11] !== onDone || $[12] !== onKill || $[13] !== task.filesTouched.length || $[14] !== task.sessionsReviewing || $[15] !== task.status || $[16] !== task.turns) {
    ...
    if ($[35] !== task.filesTouched.length) {
      t19 = task.filesTouched.length > 0 && <>{" "}· 触及 {task.filesTouched.length} 个文件</>;
    ...
    if ($[37] !== elapsedTime || $[38] !== t18 || $[39] !== t19 || $[40] !== task.sessionsReviewing) {
      t9 = <Text dimColor={true}>{elapsedTime} · 正在审查 {t17} 个会话{t19}</Text>;
      $[37] = elapsedTime;
      $[38] = t18;
      $[39] = t19;
    ...
    if ($[47] !== task.status) {
      t6 = <Text>{t20}{" "}{task.status === "running" ? <Text color="background">运行中</Text> : task.status === "completed" ? <Text color="success">已完成</Text> : <Text color="error">{task.status === "failed" ? "失败" : task.status}</Text>}</Text>;
      $[47] = task.status;
      $[48] = t6;
    } else {
      t6 = $[48];
    }
    t7 = shown.length === 0 ? <Text dimColor={true}>{task.status === "running" ? "正在启动…" : "(没有文本输出)"}</Text> : <>{hidden > 0 && <Text dimColor={true}>(前面还有 {hidden} 轮)</Text>}{shown.map(_temp2)}</>;
    $[8] = elapsedTime;
    $[9] = handleKeyDown;
    $[10] = onBack;
    $[11] = onDone;
