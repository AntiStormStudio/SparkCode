...
export function AsyncAgentDetailDialog(t0) {
  ...
  let t10;
  if ($[16] !== tokenCount) {
    t10 = tokenCount !== undefined && tokenCount > 0 && <> · {formatNumber(tokenCount)} 个 token</>;
    $[16] = tokenCount;
    $[17] = t10;
  } else {
    t10 = $[17];
  ...
  let t11;
  if ($[18] !== toolUseCount) {
    t11 = toolUseCount !== undefined && toolUseCount > 0 && <>{" "}· {toolUseCount} 次工具调用</>;
    $[18] = toolUseCount;
    $[19] = t11;
  } else {
    t11 = $[19];
  ...
  if ($[27] !== agent.status || $[28] !== onBack || $[29] !== onKillAgent) {
    t14 = exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline>{onBack && <KeyboardShortcutHint shortcut={"\u2190"} action="返回" />}<KeyboardShortcutHint shortcut="Esc/Enter/Space" action="关闭" />{agent.status === "running" && onKillAgent && <KeyboardShortcutHint shortcut="x" action="停止" />}</Byline>;
    $[27] = agent.status;
    $[28] = onBack;
    $[29] = onKillAgent;
    $[30] = t14;
