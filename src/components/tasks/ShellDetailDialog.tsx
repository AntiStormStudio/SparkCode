...
export function ShellDetailDialog(t0) {
  ...
  const t9 = isMonitor ? "监视器详情" : "Shell 详情";
  let t10;
  ...
  if ($[24] !== shell.result || $[25] !== shell.status) {
    t12 = <Text>{t11}{" "}{shell.status === "running" ? <Text color="background">运行中{shell.result?.code !== undefined && ` (退出码：${shell.result.code})`}</Text> : shell.status === "completed" ? <Text color="success">已完成{shell.result?.code !== undefined && ` (退出码：${shell.result.code})`}</Text> : <Text color="error">{shell.status === "failed" ? "失败" : shell.status}{shell.result?.code !== undefined && ` (退出码：${shell.result.code})`}</Text>}</Text>;
    $[24] = shell.result;
    $[25] = shell.status;
    $[26] = t12;
  } else {
...
function ShellOutputContent(t0) {
  ...
  if (!content) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
      t1 = <Text dimColor={true}>没有可用输出</Text>;
      $[0] = t1;
    } else {
      t1 = $[0];
    }
