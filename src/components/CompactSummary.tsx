...
export function CompactSummary(t0) {
  ...
  if (metadata) {
    ...
    if ($[4] !== isTranscriptMode || $[5] !== metadata) {
      t4 = !isTranscriptMode && <MessageResponse><Box flexDirection="column"><Text dimColor={true}>已摘要 {metadata.messagesSummarized} 条消息{" "}{metadata.direction === "up_to" ? "直到此处" : "从此处开始"}</Text>{metadata.userContext && <Text dimColor={true}>上下文：{"\u201C"}{metadata.userContext}{"\u201D"}</Text>}<Text dimColor={true}><ConfigurableShortcutHint action="app:toggleTranscript" context="Global" fallback="ctrl+o" description="展开历史" parens={true} /></Text></Box></MessageResponse>;
      $[4] = isTranscriptMode;
      $[5] = metadata;
      $[6] = t4;
    } else {
