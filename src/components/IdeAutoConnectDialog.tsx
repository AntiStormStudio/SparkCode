...
export function IdeAutoConnectDialog(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <Text dimColor={true}>也可以在 /config 中配置，或使用 --ide 参数</Text>;
    $[5] = t4;
  } else {
    t4 = $[5];
  }
  ...
  if ($[6] !== onComplete || $[7] !== t3) {
    t5 = <Dialog title="是否启用 IDE 自动连接？" color="ide" onCancel={onComplete}>{t3}{t4}</Dialog>;
    $[6] = onComplete;
    $[7] = t3;
    $[8] = t5;
  } else {
...
export function IdeDisableAutoConnectDialog(t0) {
  ...
  if ($[7] !== handleCancel || $[8] !== t4) {
    t5 = <Dialog title="是否禁用 IDE 自动连接？" subtitle="也可以在 /config 中配置" onCancel={handleCancel} color="ide">{t4}</Dialog>;
    $[7] = handleCancel;
    $[8] = t4;
    $[9] = t5;
  } else {
