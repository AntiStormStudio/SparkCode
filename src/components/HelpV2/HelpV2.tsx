...
export function HelpV2(t0) {
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <Tab key="general" id="general" title="概览"><General /></Tab>;
    $[8] = t4;
  } else {
    t4 = $[8];
  }
  ...
  if ($[9] !== antOnlyCommands || $[10] !== builtinCommands || $[11] !== close || $[12] !== columns || $[13] !== customCommands || $[14] !== maxHeight) {
    ...
    if ($[16] !== builtinCommands || $[17] !== close || $[18] !== columns || $[19] !== maxHeight) {
      t5 = <Tab key="commands" id="commands" title="命令"><Commands commands={builtinCommands} maxHeight={maxHeight} columns={columns} title="浏览默认命令：" onCancel={close} /></Tab>;
      $[16] = builtinCommands;
      $[17] = close;
      $[18] = columns;
      $[19] = maxHeight;
    ...
    if ($[21] !== close || $[22] !== columns || $[23] !== customCommands || $[24] !== maxHeight) {
      t6 = <Tab key="custom" id="custom" title="自定义命令"><Commands commands={customCommands} maxHeight={maxHeight} columns={columns} title="浏览自定义命令：" emptyMessage="未找到自定义命令" onCancel={close} /></Tab>;
      $[21] = close;
      $[22] = columns;
      $[23] = customCommands;
      $[24] = maxHeight;
    ...
    if (false && antOnlyCommands.length > 0) {
      ...
      if ($[26] !== antOnlyCommands || $[27] !== close || $[28] !== columns || $[29] !== maxHeight) {
        t7 = <Tab key="ant-only" id="ant-only" title="[仅 ANT]"><Commands commands={antOnlyCommands} maxHeight={maxHeight} columns={columns} title="浏览仅 ANT 命令：" onCancel={close} /></Tab>;
        $[26] = antOnlyCommands;
        $[27] = close;
        $[28] = columns;
        $[29] = maxHeight;
  ...
  let t6;
  if ($[31] !== tabs) {
    t6 = <Tabs title={false ? "/help" : `Spark Code v${MACRO.VERSION}`} color="professionalBlue" defaultTab="general">{tabs}</Tabs>;
    $[31] = tabs;
    $[32] = t6;
  } else {
    t6 = $[32];
