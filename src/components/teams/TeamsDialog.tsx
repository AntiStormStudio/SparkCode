  ...
  useInput((input, key) => {
    ...
    if (input === 's') {
      if (dialogLevel.type === 'teammateList' && teammateStatuses[selectedIndex]) {
        ...
        void sendShutdownRequestToMailbox(teammate.name, dialogLevel.teamName, '团队负责人请求平滑关闭');
      } else if (dialogLevel.type === 'teammateDetail' && currentTeammate) {
        void sendShutdownRequestToMailbox(currentTeammate.name, dialogLevel.teamName, '团队负责人请求平滑关闭');
        goBackToList();
      }
      return;
    }
...
function TeamDetailView(t0) {
  ...
  if ($[8] !== cycleModeShortcut) {
    t4 = <Box marginLeft={1}><Text dimColor={true}>{figures.arrowUp}/{figures.arrowDown} 选择 · Enter 查看 · k 终止 · s 平滑关闭 · p 清理空闲{supportsHideShow && " \xB7 h 隐藏/显示 \xB7 H 全部隐藏/显示"}{" \xB7 "}{cycleModeShortcut} 为全部成员同步轮换模式 · Esc 关闭</Text></Box>;
    $[8] = cycleModeShortcut;
    $[9] = t4;
  } else {
    t4 = $[9];
...
function TeammateDetailView(t0) {
  ...
  if ($[34] !== cycleModeShortcut) {
    t12 = <Box marginLeft={1}><Text dimColor={true}>{figures.arrowLeft} 返回 · Esc 关闭 · k 终止 · s 平滑关闭{getCachedBackend()?.supportsHideShow && " \xB7 h 隐藏/显示"}{" \xB7 "}{cycleModeShortcut} 轮换模式</Text></Box>;
