...
export function AgentDetail(t0) {
  ...
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text><Text bold={true}>描述</Text>（告诉 Claude 什么时候使用这个 Agent）：</Text>;
    $[9] = t10;
  } else {
    t10 = $[9];
  }
  ...
  let t21;
  if ($[28] !== agent.skills) {
    t21 = agent.skills && agent.skills.length > 0 && <Text><Text bold={true}>Skills</Text>：{" "}{agent.skills.length > 10 ? `${agent.skills.length} 个 Skill` : agent.skills.join(", ")}</Text>;
    $[28] = agent.skills;
    $[29] = t21;
  } else {
    t21 = $[29];
