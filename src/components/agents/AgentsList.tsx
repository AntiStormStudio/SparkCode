...
export function AgentsList(t0) {
  ...
  if ($[2] !== isCreateNewSelected) {
    t2 = () => <Box><Text color={isCreateNewSelected ? "suggestion" : undefined}>{isCreateNewSelected ? `${figures.pointer} ` : "  "}</Text><Text color={isCreateNewSelected ? "suggestion" : undefined}>新建 Agent</Text></Box>;
    ...
    bb1: {
      ...
      if (hasNoAgents) {
        ...
        if ($[58] === Symbol.for("react.memo_cache_sentinel")) {
          t24 = <Text dimColor={true}>未找到 Agent。你可以创建专门的子 Agent，让 Claude 委派任务。</Text>;
          t25 = <Text dimColor={true}>每个子 Agent 都有独立上下文窗口、自定义系统提示和专用工具。</Text>;
          t26 = <Text dimColor={true}>可尝试创建：代码审查、代码简化、安全审查、技术负责人或 UX 审查 Agent。</Text>;
          $[58] = t24;
          $[59] = t25;
          $[60] = t26;
        } else {
      ...
      t16 = source === "all" ? <>{AGENT_SOURCE_GROUPS.filter(_temp9).map(t24 => {
        ...
        })}{builtInAgents_0.length > 0 && <Box flexDirection="column" marginBottom={1} paddingLeft={2}><Text dimColor={true}><Text bold={true}>内置 Agent</Text>（始终可用）</Text>{builtInAgents_0.map(renderAgent)}</Box>}</> : source === "built-in" ? <><Text dimColor={true} italic={true}>内置 Agent 默认提供，无法修改。</Text><Box marginTop={1} flexDirection="column">{sortedAgents.map(agent_2 => renderAgent(agent_2))}</Box></> : <>{sortedAgents.filter(_temp0).map(agent_3 => renderAgent(agent_3))}{sortedAgents.some(_temp1) && <><Divider />{renderBuiltInAgentsSection()}</>}</>;
    }
    $[30] = changes;
    $[31] = handleKeyDown;
    $[32] = onBack;
