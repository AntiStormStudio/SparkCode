...
const WORKFLOWS: WorkflowOption[] = [{
  ...
  label: '@Spark Code - 在 issue 和 PR 评论中标记 @claude'
}, {
  value: 'claude-review' as const,
  label: 'Spark Code 审查 - 自动审查新 PR'
}];
...
export function WorkflowMultiselectDialog(t0) {
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <Box><Text dimColor={true}>更多 workflow 示例（issue 分流、CI 修复等）：{" "}<Link url="https://github.com/anthropics/claude-code-action/blob/main/examples/">https://github.com/anthropics/claude-code-action/blob/main/examples/</Link></Text></Box>;
    $[4] = t4;
  } else {
    t4 = $[4];
  }
  ...
  let t7;
  if ($[9] !== showError) {
    t7 = showError && <Box><Text color="error">请至少选择一个 workflow 后继续</Text></Box>;
    $[9] = showError;
    $[10] = t7;
  } else {
    t7 = $[10];
  ...
  let t8;
  if ($[11] !== t6 || $[12] !== t7) {
    t8 = <Dialog title="选择要安装的 GitHub workflow" subtitle="每选择一个，我们都会在你的仓库中创建对应的 workflow 文件。" onCancel={handleCancel} inputGuide={renderInputGuide}>{t4}{t6}{t7}</Dialog>;
    $[11] = t6;
    $[12] = t7;
    $[13] = t8;
  } else {
