...
export function MessageSelector({
  ...
  return <Box flexDirection="column" width="100%">
      ...
      <Box flexDirection="column" marginX={1} gap={1}>
        ...
        {!hasMessagesToSelect && <>
            <Text>暂时没有可回退的内容。</Text>
          </>}
        {!error && messageToRestore && hasMessagesToSelect && <>
            <Text>
              请确认要恢复{!diffStatsForRestore && '对话'}到发送这条消息之前：
            </Text>
            ...
            {canRestoreCode_0 && <Box marginBottom={1}>
                <Text dimColor>
                  {figures.warning} 回退不会影响手动或通过 bash 编辑的文件。
                </Text>
              </Box>}
          </>}
        {showPickList && <>
...
function getRestoreOptionConversationText(option: RestoreOption): string {
  switch (option) {
    ...
    case 'summarize_up_to':
      return '前面的消息将被总结。此消息及之后的消息保持不变，你会留在对话末尾。';
    case 'both':
    case 'conversation':
      return '对话将被分叉。';
    case 'code':
...
function RestoreCodeConfirmation(t0) {
  ...
  if (!diffStatsForRestore.filesChanged || !diffStatsForRestore.filesChanged[0]) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
      t1 = <Text dimColor={true}>代码没有更改（无需恢复）。</Text>;
      $[0] = t1;
    } else {
      t1 = $[0];
    }
  ...
  if (numFilesChanged === 1) {
    ...
    if (numFilesChanged === 2) {
      ...
      const file1_0 = t1;
      fileLabel = `${file1_0} 和其他 ${diffStatsForRestore.filesChanged.length - 1} 个文件`;
    }
  }
  let t1;
  ...
  if ($[11] !== fileLabel || $[12] !== t1) {
    t2 = <><Text dimColor={true}>将恢复 {fileLabel} 中的代码变更{" "}{t1}。</Text></>;
    $[11] = fileLabel;
    $[12] = t1;
    $[13] = t2;
  } else {
...
function UserMessageOption(t0) {
  ...
  if ($[3] !== color || $[4] !== columns || $[5] !== content || $[6] !== dimColor || $[7] !== lastBlock || $[8] !== paddingRight) {
    ...
    bb0: {
      const rawMessageText = typeof content === "string" ? content.trim() : lastBlock && isTextBlock(lastBlock) ? lastBlock.text.trim() : "（无提示）";
