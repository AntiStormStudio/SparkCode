...
export function ThinkingToggle(t0) {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t1 = [{
      value: "true",
      label: "已启用",
      description: "Spark 会先思考再回复"
    }, {
      value: "false",
      label: "已停用",
      description: "Spark 会直接回复，不进行扩展思考"
    }];
    $[0] = t1;
  } else {
    t1 = $[0];
  ...
  if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = <Box marginBottom={1} flexDirection="column"><Text color="remember" bold={true}>切换思考模式</Text><Text dimColor={true}>为本次会话启用或停用思考。</Text></Box>;
    $[14] = t8;
  } else {
    t8 = $[14];
  }
  ...
  if ($[15] !== confirmationPending || $[16] !== currentValue || $[17] !== handleSelectChange || $[18] !== onCancel) {
    t9 = <Box flexDirection="column">{t8}{confirmationPending !== null ? <Box flexDirection="column" marginBottom={1} gap={1}><Text color="warning">在对话中途切换思考模式会增加延迟，并可能降低质量。建议在会话开始时设置。</Text><Text color="warning">要继续吗？</Text></Box> : <Box flexDirection="column" marginBottom={1}><Select defaultValue={currentValue ? "true" : "false"} defaultFocusValue={currentValue ? "true" : "false"} options={options} onChange={handleSelectChange} onCancel={onCancel ?? _temp} visibleOptionCount={2} /></Box>}</Box>;
  ...
  if ($[20] !== confirmationPending || $[21] !== exitState.keyName || $[22] !== exitState.pending) {
    t10 = <Text dimColor={true} italic={true}>{exitState.pending ? <>再次按 {exitState.keyName} 退出</> : confirmationPending !== null ? <Byline><KeyboardShortcutHint shortcut="Enter" action="确认" /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="取消" /></Byline> : <Byline><KeyboardShortcutHint shortcut="Enter" action="确认" /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description="退出" /></Byline>}</Text>;
