...
export function SandboxDoctorSection() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    bb0: {
      ...
      t0 = <Box flexDirection="column"><Text bold={true}>Sandbox</Text><Text>└ 状态：<Text color={statusColor}>{statusText}</Text></Text>{depCheck.errors.map(_temp)}{depCheck.warnings.map(_temp2)}{hasErrors && <Text dimColor={true}>└ 运行 /sandbox 查看安装说明</Text>}</Box>;
    }
    $[0] = t0;
    $[1] = t1;
  } else {
