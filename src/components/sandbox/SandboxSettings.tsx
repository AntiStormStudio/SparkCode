...
export function SandboxSettings(t0) {
  ...
  const t3 = currentMode === "auto-allow" ? `Sandbox BashTool，自动允许 ${currentIndicator}` : "Sandbox BashTool，自动允许";
  let t4;
  if ($[3] !== t3) {
    t4 = {
      label: t3,
  ...
  const t5 = currentMode === "regular" ? `Sandbox BashTool，常规权限 ${currentIndicator}` : "Sandbox BashTool，常规权限";
  let t6;
  if ($[5] !== t5) {
    t6 = {
      label: t5,
  ...
  const t7 = currentMode === "disabled" ? `不使用 Sandbox ${currentIndicator}` : "不使用 Sandbox";
  let t8;
  if ($[7] !== t7) {
    t8 = {
      label: t7,
        ...
        case "regular":
          {
            ...
            onComplete("\u2713 Sandbox 已启用，bash 命令使用常规权限");
            break bb33;
          }
        case "disabled":
          {
  ...
  if ($[18] !== handleSelect || $[19] !== onComplete || $[20] !== options || $[21] !== showSocketWarning) {
    t13 = <Tab key="mode" id="Mode" title="模式"><SandboxModeTab showSocketWarning={showSocketWarning} options={options} onSelect={handleSelect} onComplete={onComplete} /></Tab>;
    $[18] = handleSelect;
    $[19] = onComplete;
    $[20] = options;
  ...
  let t14;
  if ($[23] !== onComplete) {
    t14 = <Tab key="overrides" id="Overrides" title="覆盖项"><SandboxOverridesTab onComplete={onComplete} /></Tab>;
    $[23] = onComplete;
    $[24] = t14;
  } else {
    t14 = $[24];
  ...
  if ($[25] === Symbol.for("react.memo_cache_sentinel")) {
    t15 = <Tab key="config" id="Config" title="配置"><SandboxConfigTab /></Tab>;
    $[25] = t15;
  } else {
    t15 = $[25];
  }
  ...
  if ($[26] !== depCheck || $[27] !== hasErrors || $[28] !== hasWarnings || $[29] !== modeTab || $[30] !== overridesTab) {
    t16 = hasErrors ? [<Tab key="dependencies" id="Dependencies" title="依赖"><SandboxDependenciesTab depCheck={depCheck} /></Tab>] : [modeTab, ...(hasWarnings ? [<Tab key="dependencies" id="Dependencies" title="依赖"><SandboxDependenciesTab depCheck={depCheck} /></Tab>] : []), overridesTab, configTab];
    $[26] = depCheck;
    $[27] = hasErrors;
    $[28] = hasWarnings;
    $[29] = modeTab;
...
function SandboxModeTab(t0) {
  ...
  if ($[0] !== showSocketWarning) {
    t1 = showSocketWarning && <Box marginBottom={1}><Text color="warning">无法阻止 Unix domain socket（见“依赖”标签）</Text></Box>;
    $[0] = showSocketWarning;
    $[1] = t1;
  } else {
    t1 = $[1];
  ...
  if ($[11] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Text dimColor={true}><Text bold={true} dimColor={true}>自动允许模式：</Text>{" "}命令会优先自动在 Sandbox 中运行；如果需要在 Sandbox 外运行，会回退到常规权限。显式 ask/deny 规则始终生效。</Text>;
    $[11] = t5;
  } else {
    t5 = $[11];
  }
