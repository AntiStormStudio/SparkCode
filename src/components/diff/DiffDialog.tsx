...
export function DiffDialog(t0) {
  ...
  if ($[38] !== diffData.stats) {
    t17 = diffData.stats ? <Text dimColor={true}>{diffData.stats.filesCount} 个文件已变更{diffData.stats.linesAdded > 0 && <Text color="diffAddedWord"> +{diffData.stats.linesAdded}</Text>}{diffData.stats.linesRemoved > 0 && <Text color="diffRemovedWord"> -{diffData.stats.linesRemoved}</Text>}</Text> : null;
    $[38] = diffData.stats;
    $[39] = t17;
  } else {
    t17 = $[39];
  ...
  const subtitle = t17;
  const headerTitle = currentTurn ? `第 ${currentTurn.turnIndex} 轮` : "未提交更改";
  ...
  if ($[51] !== dismissShortcut || $[52] !== sources.length || $[53] !== viewMode) {
    t23 = exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : viewMode === "list" ? <Byline>{sources.length > 1 && <Text>←/→ 来源</Text>}<Text>↑/↓ 选择</Text><Text>Enter 查看</Text><Text>{dismissShortcut} 关闭</Text></Byline> : <Byline><Text>← 返回</Text><Text>{dismissShortcut} 关闭</Text></Byline>;
    $[51] = dismissShortcut;
    $[52] = sources.length;
    $[53] = viewMode;
    $[54] = t23;
