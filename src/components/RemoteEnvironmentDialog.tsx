...
const SETUP_HINT = `请在此配置环境：https://spark-ai.top/code`;
type Props = {
...
function EnvironmentLabel(t0) {
  ...
  let t3;
  if ($[4] !== t1 || $[5] !== t2) {
    t3 = <Text>{figures.tick} 当前使用 {t1}{" "}{t2}</Text>;
    $[4] = t1;
    $[5] = t2;
    $[6] = t3;
  } else {
...
function MultipleEnvironmentsContent(t0) {
  ...
  if ($[8] !== environments || $[9] !== loadingState || $[10] !== onSelect || $[11] !== selectedEnvironment.environment_id) {
    t5 = loadingState === "updating" ? <LoadingState message={"更新中\u2026"} /> : <Select options={environments.map(_temp)} defaultValue={selectedEnvironment.environment_id} onChange={onSelect} onCancel={() => onSelect("cancel")} layout="compact-vertical" />;
    $[8] = environments;
    $[9] = loadingState;
    $[10] = onSelect;
