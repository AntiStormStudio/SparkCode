...
export const AUTO_MODE_DESCRIPTION = "自动模式会让 Spark 自动处理权限提示：执行前，Spark 会检查每次工具调用是否存在高风险操作和提示注入。Spark 判断为安全的操作会执行，判断为有风险的操作会被阻止，Spark 可能会尝试其他方案。此模式适合长时间任务，会让会话费用略高。Spark 也可能出错并导致有害命令运行，建议只在隔离环境中使用。按 Shift+Tab 可切换模式。";
type Props = {
  onAccept(): void;
  onDecline(): void;
