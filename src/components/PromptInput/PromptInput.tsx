  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: '这条提示将会在 Spark Code 网页端启动一个 ultraplan 会话',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  useEffect(() => {
    if (isUltrareviewEnabled() && ultrareviewTriggers.length) {
      addNotification({
        ...
        text: 'Spark 完成后运行 /ultrareview，可在云端审阅这些改动',
        priority: 'immediate',
        timeoutMs: 5000
      });
    }
  ...
  const handleImagePaste = useCallback(() => {
    void getImageFromClipboard().then(imageData => {
      if (imageData) {
        ...
        const message = env.isSSH() ? "剪贴板里没有图片。当前是 SSH 环境，可以试试 scp。" : `剪贴板里没有图片。使用 ${shortcutDisplay} 粘贴图片。`;
        addNotification({
  ...
  useInput((char, key) => {
    ...
    if (getPlatform() === 'macos' && isMacosOptionChar(char)) {
      ...
      const jsx = terminalName ? <Text dimColor>
          要启用 {shortcut}，请在 {terminalName} 偏好设置中打开 <Text bold>Option as Meta</Text>（⌘,）
