...
export function IdeStatusIndicator(t0) {
  ...
  if (ideSelection.text && ideSelection.lineCount > 0) {
    ...
    if ($[0] !== ideSelection.lineCount || $[1] !== t1) {
      t2 = <Text color="ide" key="selection-indicator" wrap="truncate">⧉ 已选中 {ideSelection.lineCount} {t1}</Text>;
  ...
  if (ideSelection.filePath) {
    ...
    let t2;
    if ($[5] !== t1) {
      t2 = <Text color="ide" key="selection-indicator" wrap="truncate">⧉ 位于 {t1}</Text>;
      $[5] = t1;
      $[6] = t2;
    } else {
      t2 = $[6];
