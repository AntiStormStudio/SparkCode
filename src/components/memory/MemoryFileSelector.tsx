...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), "SPARK.md");
  const projectMemoryPath = join(getOriginalCwd(), "SPARK.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.claude/SPARK.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./SPARK.md`;
      } else {
        if (file.parent) {
  ...
  if (isAutoMemoryEnabled()) {
    ...
    for (const agent of agentDefinitions.activeAgents) {
      if (agent.memory) {
        ...
        folderOptions.push({
          label: `打开 ${chalk.bold(agent.agentType)} agent 记忆`,
