...
export function getDownloadUrl(): string {
  switch (process.platform) {
    case 'win32':
      return 'https://spark-ai.top/api/desktop/win32/x64/exe/latest/redirect';
    default:
      return 'https://spark-ai.top/api/desktop/darwin/universal/dmg/latest/redirect';
  }
}
...
export function DesktopHandoff(t0) {
    ...
    t1 = input => {
      ...
      if (state === "prompt-download") {
        if (input === "y" || input === "Y") {
          ...
          if (input === "n" || input === "N") {
            onDone(`/desktop 需要桌面应用支持。了解更多：${DESKTOP_DOCS_URL}`, {
              display: "system"
            });
          }
        }
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("未检测到 Claude Desktop。");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`Claude Desktop 版本过低（当前 v${installStatus.version}，需要 v1.1.2396+）。`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      ...
      success: "正在于 Claude Desktop 中打开\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
