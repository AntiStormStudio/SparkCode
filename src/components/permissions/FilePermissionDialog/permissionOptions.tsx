...
/**
 * Check if a path is within the project's .sparkc/ folder.
 * This is used to determine whether to show the special ".sparkc folder" permission option.
 */
export function isInClaudeFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath);
  const claudeFolderPath = expandPath(`${getOriginalCwd()}/.sparkc`);

  // Check if the path is within the project's .sparkc folder
...
/**
 * Check if a path is within the global ~/.sparkc/ folder.
 * This is used to determine whether to show the special ".sparkc folder" permission option
...
export function getFilePermissionOptions({
  ...
  // Check if this is a .sparkc/ folder path (project or global)
  const inClaudeFolder = isInClaudeFolder(filePath);
  const inGlobalClaudeFolder = isInGlobalClaudeFolder(filePath);

  // Option 2: For .sparkc/ folder, show special option instead of generic session option
  // Note: Session-level options are always shown since they only affect in-memory state,
  // not persisted settings. The allowManagedPermissionRulesOnly setting only restricts
  // persisted permission rules.
  if ((inClaudeFolder || inGlobalClaudeFolder) && operationType !== 'read') {
    options.push({
      label: '是，并允许 Claude 在本会话中编辑自己的设置',
    ...
    if (inAllowedPath) {
      ...
      if (operationType === 'read') {
        sessionLabel = <Text>
            是，允许本会话读取 <Text bold>{dirName}/</Text>
          </Text>;
      } else {
        sessionLabel = <Text>
            是，允许本会话编辑 <Text bold>{dirName}/</Text> 中的所有内容 <Text bold>({modeCycleShortcut})</Text>
          </Text>;
      }
    }
    options.push({
