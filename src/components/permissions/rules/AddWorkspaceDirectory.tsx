...
function PermissionDescription() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t0 = <Text dimColor={true}>Spark Code 将可以读取此目录中的文件，并在启用自动接受编辑时进行修改。</Text>;
    $[0] = t0;
  } else {
    t0 = $[0];
  }
...
function _temp2(exitState) {
  return exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline><KeyboardShortcutHint shortcut="Tab" action="complete" /><KeyboardShortcutHint shortcut="Enter" action="add" /><ConfigurableShortcutHint action="confirm:no" context="Settings" fallback="Esc" description="取消" /></Byline>;
}
