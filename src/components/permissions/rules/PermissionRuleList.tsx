...
function RuleDetails(t0) {
  ...
  if (rule.source === "policySettings") {
    ...
    if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
      t9 = <Text italic={true}>这条规则由托管设置配置，无法修改。{"\n"}如需更多信息，请联系系统管理员。</Text>;
      $[17] = t9;
    } else {
      t9 = $[17];
    }
...
function PermissionRulesTab(t0) {
  ...
  if ($[0] !== t0) {
    ...
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
      t8 = {
        allow: "Spark Code 使用已允许的工具前不会再询问。",
        ask: "Spark Code 使用这些工具前会始终请求确认。",
        deny: "Spark Code 会始终拒绝使用已拒绝的工具。"
      };
      $[10] = t8;
    } else {
      t8 = $[10];
...
export function PermissionRuleList(t0) {
  ...
  if ($[30] !== changes || $[31] !== onExit || $[32] !== onRetryDenials) {
    t18 = () => {
      ...
      if (retryDenials.length > 0) {
        ...
        onExit(undefined, {
          shouldQuery: true,
          metaMessages: [`已授权：${commands.join(", ")}。现在可以重试${commands.length === 1 ? "这条命令" : "这些命令"}。`]
        });
        return;
      }
  ...
  if ($[89] === Symbol.for("react.memo_cache_sentinel")) {
    t28 = <Text>Spark Code 可以读取工作区中的文件，并在启用自动接受编辑时进行修改。</Text>;
    $[89] = t28;
  } else {
    t28 = $[89];
  }
  ...
  if ($[101] !== defaultTab || $[102] !== exitState.keyName || $[103] !== exitState.pending || $[104] !== headerFocused || $[105] !== isSearchMode) {
    t31 = <Box marginTop={1} paddingLeft={1}><Text dimColor={true}>{exitState.pending ? <>再次按 {exitState.keyName} 退出</> : headerFocused ? <>←/→ 切换标签 · ↓ 返回 · Esc 取消</> : isSearchMode ? <>输入筛选 · Enter/↓ 选择 · ↑ 标签 · Esc 清除</> : hasDenials && defaultTab === "recent" ? <>Enter 批准 · r 重试 · ↑↓ 导航 · ←/→ 切换 · Esc 取消</> : <>↑↓ 导航 · Enter 选择 · 输入搜索 · ←/→ 切换 · Esc 取消</>}</Text></Box>;
    $[101] = defaultTab;
