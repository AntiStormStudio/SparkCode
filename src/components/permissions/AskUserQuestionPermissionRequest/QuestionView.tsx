...
export function QuestionView(t0) {
  ...
  if ($[47] !== isInPlanMode || $[48] !== planFilePath) {
    t8 = isInPlanMode && planFilePath && <Box flexDirection="column" gap={0}><Divider color="inactive" /><Text color="inactive">计划：<FilePathLink filePath={planFilePath} /></Text></Box>;
    $[47] = isInPlanMode;
    $[48] = planFilePath;
    $[49] = t8;
  } else {
  ...
  let t17;
  if ($[80] !== t15 || $[81] !== t16) {
    t17 = <Text color={t15}>{t16}. 继续和 Claude 讨论</Text>;
    $[80] = t15;
    $[81] = t16;
    $[82] = t17;
  } else {
  ...
  if ($[86] !== footerIndex || $[87] !== isFooterFocused || $[88] !== isInPlanMode || $[89] !== options.length) {
    t19 = isInPlanMode && <Box flexDirection="row" gap={1}>{isFooterFocused && footerIndex === 1 ? <Text color="suggestion">{figures.pointer}</Text> : <Text> </Text>}<Text color={isFooterFocused && footerIndex === 1 ? "suggestion" : undefined}>{options.length + 2}. 跳过问题并立即规划</Text></Box>;
    $[86] = footerIndex;
    $[87] = isFooterFocused;
    $[88] = isInPlanMode;
    $[89] = options.length;
  ...
  if ($[94] !== questions.length) {
    t21 = questions.length === 1 ? <>用 {figures.arrowUp}/{figures.arrowDown} 导航</> : "Tab/方向键导航";
    $[94] = questions.length;
    $[95] = t21;
  } else {
    t21 = $[95];
