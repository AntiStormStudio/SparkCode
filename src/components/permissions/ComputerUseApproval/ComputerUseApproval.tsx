...
function ComputerUseTccPanel(t0) {
  ...
  if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = <Text dimColor={true}>请在系统设置中授予缺失权限，然后选择“重试”。授予屏幕录制权限后，macOS 可能要求你重启 Spark Code。</Text>;
    $[15] = t7;
  } else {
    t7 = $[15];
  }
...
function ComputerUseAppListPanel(t0) {
  ...
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = {
      label: <Text>拒绝，并告诉 Claude 需要怎么改 <Text bold={true}>(esc)</Text></Text>,
      value: "deny"
    };
    $[9] = t8;
  } else {
  ...
  if ($[30] !== request.willHide) {
    t16 = request.willHide && request.willHide.length > 0 ? <Text dimColor={true}>Claude 工作时将隐藏另外 {request.willHide.length} 个应用。</Text> : null;
    $[30] = request.willHide;
    $[31] = t16;
  } else {
    t16 = $[31];
  ...
  let t21;
  if ($[45] !== t11 || $[46] !== t20) {
    t21 = <Dialog title="Computer Use 想控制这些应用" onCancel={t11}>{t20}</Dialog>;
    $[45] = t11;
    $[46] = t20;
    $[47] = t21;
  } else {
