...
export function SkillPermissionRequest(props) {
  ...
  if ($[6] !== skill) {
    ...
    if (showAlwaysAllowOptions) {
      ...
      if ($[9] !== t5) {
        t7 = {
          label: <Text>是，并且在 {t6} 中不再询问 {t5}</Text>,
          value: "yes-exact"
        };
        $[9] = t5;
        $[10] = t7;
      ...
      if (spaceIndex > 0) {
        ...
        if ($[14] !== t9) {
          t11 = {
            label: <Text>是，并且在 {t10} 中不再询问 {t9} 命令</Text>,
            value: "yes-prefix"
          };
          $[14] = t9;
          $[15] = t11;
  ...
  const t12 = `使用 Skill "${skill}"？`;
  let t13;
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t13 = <Text>Spark 可能会使用此 Skill 中的说明、代码或文件。</Text>;
    $[33] = t13;
  } else {
    t13 = $[33];
  }
...
function _temp(input) {
  ...
  if (!result.success) {
    logError(new Error(`解析 skill 工具输入失败：${result.error.message}`));
    return "";
  }
