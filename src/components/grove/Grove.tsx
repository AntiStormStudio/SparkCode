...
function GracePeriodContentBody() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t0 = <Text>我们的消费者条款和隐私政策更新将于{" "}<Text bold={true}>2025 年 10 月 8 日</Text>生效。你现在可以接受更新后的条款。</Text>;
    $[0] = t0;
  } else {
    t0 = $[0];
  }
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <Box paddingLeft={1}><Text>{t2}{t3}<Text>— 允许使用你的聊天和编码会话来训练并改进 Anthropic AI 模型。你可以随时在隐私设置中更改（<Link url="https://spark-ai.top/settings/data-privacy-controls" />）。</Text></Text></Box>;
    $[4] = t4;
  } else {
    t4 = $[4];
  }
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Box flexDirection="column">{t1}{t4}<Box paddingLeft={1}><Text><Text>· </Text><Text bold={true}>数据保留更新 </Text><Text>— 为了帮助我们改进 AI 模型和安全保护，数据保留期限将延长至 5 年。</Text></Text></Box></Box>;
    $[5] = t5;
  } else {
    t5 = $[5];
  }
...
function PostGracePeriodContentBody() {
  ...
  let t2;
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box flexDirection="column"><Text bold={true}>帮助改进 Claude</Text><Text>允许使用你的聊天和编码会话来训练并改进 Anthropic AI 模型。你可以随时在隐私设置中更改。</Text><Link url="https://spark-ai.top/settings/data-privacy-controls" /></Box>;
    $[2] = t2;
  } else {
    t2 = $[2];
  }
  ...
  if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}>{t1}{t2}<Box flexDirection="column"><Text bold={true}>这会如何影响数据保留</Text><Text>开启“改进 Claude”会把数据保留期限从 30 天延长到 5 年；关闭则保持默认 30 天。你可以随时删除数据。</Text></Box></Box>;
    $[3] = t3;
  } else {
    t3 = $[3];
  }
...
export function GroveDialog(t0) {
  ...
  if ($[8] !== groveConfig?.domain_excluded) {
    t4 = groveConfig?.domain_excluded ? [{
      label: "接受条款 · 帮助改进 Claude：关闭（适用于你域名下的邮箱）",
  ...
  if ($[18] === Symbol.for("react.memo_cache_sentinel")) {
    t9 = <Box flexDirection="column"><Text bold={true}>请选择继续方式</Text><Text>确认后，你的选择会立即生效。</Text></Box>;
    $[18] = t9;
  } else {
    t9 = $[18];
  }
...
export function PrivacySettingsDialog(t0) {
  ...
  if (domainExcluded) {
    let t4;
    if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
      t4 = <Text color="error">false（适用于你域名下的邮箱）</Text>;
      $[5] = t4;
    } else {
      t4 = $[5];
    }
  ...
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Text>在这里查看和管理你的隐私设置：{" "}<Link url="https://spark-ai.top/settings/data-privacy-controls" /></Text>;
    $[9] = t5;
  } else {
    t5 = $[9];
  }
