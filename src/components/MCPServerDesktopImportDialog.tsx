...
export function MCPServerDesktopImportDialog(t0) {
  ...
  if ($[8] !== onDone || $[9] !== scope || $[10] !== theme) {
    t6 = importedCount_0 => {
      if (importedCount_0 > 0) {
        writeToStdout(`\n${color("success", theme)(`已将 ${importedCount_0} 个 MCP 服务器导入到 ${scope} 配置。`)}\n`);
      } else {
        writeToStdout("\n没有导入服务器。");
      }
      onDone();
  ...
  const t10 = `在 Claude Desktop 中找到 ${t8} 个 MCP 服务器。`;
  let t11;
  if ($[16] !== collisions.length) {
    t11 = collisions.length > 0 && <Text color="warning">注意：部分服务器已存在同名配置。如果选中，将使用带编号的后缀导入。</Text>;
