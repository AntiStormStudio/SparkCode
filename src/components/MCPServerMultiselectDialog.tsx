...
export function MCPServerMultiselectDialog(t0) {
  ...
  const t3 = `.mcp.json 中发现 ${serverNames.length} 个新的 MCP 服务器`;
  let t4;
  ...
  if ($[14] !== handleEscRejectAll || $[15] !== t3 || $[16] !== t6) {
    t7 = <Dialog title={t3} subtitle="选择要启用的项。" color="warning" onCancel={handleEscRejectAll} hideInputGuide={true}>{t4}{t6}</Dialog>;
