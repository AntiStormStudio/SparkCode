...
export function MCPAgentServerMenu({
  ...
  const handleAuthenticate = useCallback(async () => {
    ...
    try {
      ...
      onComplete?.(`${agentServer.name} 认证成功。该服务器会在代理运行时连接。`);
    } catch (err) {
  ...
  return <Dialog title={`${capitalizedServerName} MCP 服务器`} subtitle="仅代理使用" onCancel={onCancel} inputGuide={exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline>
      ...
      <Box flexDirection="column" gap={0}>
        ...
        <Box marginTop={1}>
          <Text bold>状态： </Text>
          <Text>
            {color('inactive', theme)(figures.radioOff)} 未连接（仅代理）
          </Text>
        </Box>
      ...
      <Box>
        <Text dimColor>该服务器仅在运行代理时连接。</Text>
      </Box>
