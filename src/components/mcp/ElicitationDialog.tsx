...
function ElicitationFormDialog({
  ...
  useNotifyAfterTimeout('Spark Code needs your input', 'elicitation_dialog');
  ...
  function validateMultiSelect(fieldName: string, schema_0: PrimitiveSchemaDefinition) {
    ...
    if (min !== undefined && selected.length < min && (selected.length > 0 || fieldRequired)) {
      ...
      updateValidationError(fieldName, `最多选择 ${max} 项`);
    } else {
      ...
      if (_input === ' ') {
        ...
        if (optionValue !== undefined) {
          ...
          if (min_0 !== undefined && newSelected.length < min_0 && (newSelected.length > 0 || currentField.isRequired)) {
            updateValidationError(currentField.name, `至少选择 ${min_0} 项`);
          } else if (max_0 !== undefined && newSelected.length > max_0) {
            updateValidationError(currentField.name, `最多选择 ${max_0} 项`);
          } else {
  ...
  return <Dialog title={`MCP 服务器“${serverName}”请求你的输入`} subtitle={`\n${message}`} color="permission" onCancel={() => onResponse('cancel')} isCancelActive={(!currentField || !!focusedButton) && !expandedAccordion} inputGuide={exitState => exitState.pending ? <Text>再次按 {exitState.keyName} 退出</Text> : <Byline>
            ...
            {currentField && <KeyboardShortcutHint shortcut="Backspace" action="清空" />}
...
function ElicitationURLDialog({
  ...
  useNotifyAfterTimeout('Spark Code needs your input', 'elicitation_url_dialog');
