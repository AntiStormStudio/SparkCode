...
export function MCPStdioServerMenu({
  ...
  menuOptions.push({
    label: server.client.type !== 'disabled' ? '停用' : '启用',
  ...
  return <Box flexDirection="column">
      <Box flexDirection="column" paddingX={1} borderStyle={borderless ? undefined : 'round'}>
        ...
        <Box flexDirection="column" gap={0}>
          <Box>
            <Text bold>状态： </Text>
            {server.client.type === 'disabled' ? <Text>{color('inactive', theme)(figures.radioOff)} 已停用</Text> : server.client.type === 'connected' ? <Text>{color('success', theme)(figures.tick)} 已连接</Text> : server.client.type === 'pending' ? <>
