  ...
  if ($[10] !== agentMcpServers.length || $[11] !== filteredClients.length || $[12] !== onComplete || $[13] !== servers.length) {
    t7 = () => {
      ...
      if (servers.length === 0 && agentMcpServers.length === 0) {
        onComplete("未配置 MCP 服务器。如果这不符合预期，请运行 /doctor。也可以运行 `claude mcp --help` 或访问 https://code.claude.com/docs/en/mcp 了解更多。");
      }
    };
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "claudeai-proxy" ? "spark-ai.top" : "Spark Code";
