...
export function Stats(t0) {
  ...
  if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box marginTop={1}><Spinner /><Text> 正在加载你的 Spark Code 统计…</Text></Box>;
    $[1] = t2;
  } else {
    t2 = $[1];
  }
...
function StatsContent(t0) {
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">还没有可用统计。开始使用 Spark Code 后会显示在这里。</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
  ...
  if ($[17] !== allTimeStats || $[18] !== dateRange || $[19] !== displayStats || $[20] !== isLoadingFiltered) {
    t7 = <Tab id="Overview" title="概览"><OverviewTab stats={displayStats} allTimeStats={allTimeStats} dateRange={dateRange} isLoading={isLoadingFiltered} /></Tab>;
    $[17] = allTimeStats;
    $[18] = dateRange;
    $[19] = displayStats;
  ...
  if ($[22] !== dateRange || $[23] !== displayStats || $[24] !== isLoadingFiltered) {
    t8 = <Tab id="Models" title="模型"><ModelsTab stats={displayStats} dateRange={dateRange} isLoading={isLoadingFiltered} /></Tab>;
    $[22] = dateRange;
    $[23] = displayStats;
...
function generateFunFactoid(stats: ClaudeCodeStats, totalTokens: number): string {
  ...
  if (totalTokens > 0) {
    ...
    for (const book of matchingBooks) {
      ...
      if (times >= 2) {
        factoids.push(`你使用的 token 约为 ${book.name} 的 ${Math.floor(times)} 倍`);
      } else {
        factoids.push(`你使用的 token 大约相当于 ${book.name}`);
      }
    }
  }
...
function renderOverviewToAnsi(stats: ClaudeCodeStats): string[] {
  ...
  if (favoriteModel) {
    lines.push(row('最常用模型', renderModelName(favoriteModel[0]), 'token 总数', formatNumber(totalTokens)));
  }
  lines.push('');
  ...
  if ("external" === 'ant' && stats.totalSpeculationTimeSavedMs > 0) {
    const label = '推测节省时间：'.padEnd(COL1_LABEL_WIDTH);
  ...
  if (feature('SHOT_STATS') && stats.shotDistribution) {
    ...
    if (totalWithShots > 0) {
      ...
      lines.push('');
      lines.push('调用轮次分布');
      lines.push(row('1 次', fmtBucket(b1, pct(b1)), '2–5 次', fmtBucket(b2_5, pct(b2_5))));
      lines.push(row('6–10 次', fmtBucket(b6_10, pct(b6_10)), '11+ 次', fmtBucket(b11, pct(b11))));
  ...
  lines.push(chalk.gray(`统计范围：最近 ${stats.totalDays} 天`));
  return lines;
}
