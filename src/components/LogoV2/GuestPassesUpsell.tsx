...
export function GuestPassesUpsell() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    t0 = <Text dimColor={true}><Text color="claude">[✻]</Text> <Text color="claude">[✻]</Text>{" "}<Text color="claude">[✻]</Text> ·{" "}{reward ? `分享 Spark Code，赚取 ${formatCreditAmount(reward)} 额外用量 · /passes` : "在 /passes 查看 3 张访客通行证"}</Text>;
    $[0] = t0;
  } else {
    t0 = $[0];
  }
