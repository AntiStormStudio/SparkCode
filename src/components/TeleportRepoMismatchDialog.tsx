...
export function TeleportRepoMismatchDialog(t0) {
  ...
  if ($[8] !== availablePaths.length || $[9] !== errorMessage || $[10] !== handleChange || $[11] !== options || $[12] !== targetRepo || $[13] !== validating) {
    t3 = availablePaths.length > 0 ? <><Box flexDirection="column" gap={1}>{errorMessage && <Text color="error">{errorMessage}</Text>}<Text>在 <Text bold={true}>{targetRepo}</Text> 中打开 Spark Code：</Text></Box>{validating ? <Box><Spinner /><Text> 正在验证仓库…</Text></Box> : <Select options={options} onChange={value_0 => void handleChange(value_0)} />}</> : <Box flexDirection="column" gap={1}>{errorMessage && <Text color="error">{errorMessage}</Text>}<Text dimColor={true}>请在 {targetRepo} 的 checkout 中运行 claude --teleport</Text></Box>;
  ...
  if ($[15] !== onCancel || $[16] !== t3) {
    t4 = <Dialog title="Teleport 到仓库" onCancel={onCancel} color="background">{t3}</Dialog>;
    $[15] = onCancel;
    $[16] = t3;
    $[17] = t4;
  } else {
