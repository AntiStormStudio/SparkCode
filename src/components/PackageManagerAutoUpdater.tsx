...
export function PackageManagerAutoUpdater(t0) {
  ...
  const updateCommand = packageManager === "homebrew" ? "brew upgrade claude-code" : packageManager === "winget" ? "winget upgrade Anthropic.ClaudeCode" : packageManager === "apk" ? "apk upgrade claude-code" : "使用包管理器更新命令";
  let t4;
  if ($[3] !== verbose) {
  ...
  let t5;
  if ($[5] !== updateCommand) {
    t5 = <Text color="warning" wrap="truncate">有可用更新，请运行：<Text bold={true}>{updateCommand}</Text></Text>;
    $[5] = updateCommand;
    $[6] = t5;
  } else {
    t5 = $[6];
