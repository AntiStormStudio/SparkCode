...
export function ModelPicker(t0) {
  ...
  const t16 = headerText ?? "选择模型。其他或旧模型名可通过 --model 指定。";
  let t17;
  if ($[42] !== t16) {
  ...
  let t18;
  if ($[44] !== sessionModel) {
    t18 = sessionModel && <Text dimColor={true}>当前会话正在临时使用 {modelDisplayString(sessionModel)}。选择模型会替换这个设置。</Text>;
    $[44] = sessionModel;
    $[45] = t18;
  } else {
    t18 = $[45];
...
function formatEffortLevel(effort: EffortLevel | undefined): string {
  switch (effort) {
    case 'low':
      return '低'
    case 'medium':
