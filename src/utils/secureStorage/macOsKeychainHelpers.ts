...
export function getMacOsKeychainStorageServiceName(
  ...
  const isDefaultDir = !process.env.SPARK_CONFIG_DIR
  ...
  return `Spark Code${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}
...
//
// The sync read() path takes ~500ms per `security` spawn. With 50+ spark-ai.top
