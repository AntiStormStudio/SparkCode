...
/**
 * Resolve Spark Code-specific path patterns for sandbox-runtime.
 *
 * Spark Code uses special path prefixes in permission rules:
...
/**
 * Convert Spark Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where Spark Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.sparkc', 'settings.json'))
    denyWrite.push(resolve(cwd, '.sparkc', 'settings.local.json'))
  }
  ...
  denyWrite.push(resolve(originalCwd, '.sparkc', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.sparkc', 'skills'))
  }
