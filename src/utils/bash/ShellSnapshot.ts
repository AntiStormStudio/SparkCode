...
/**
 * Generates Spark Code specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Spark Code content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Spark Code defaults only`,
        )
      }
