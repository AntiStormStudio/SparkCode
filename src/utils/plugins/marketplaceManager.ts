/**
 * Marketplace manager for Spark Code plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Spark Code bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Spark Code version. Run 'claude marketplace remove ${name}' and ` +
