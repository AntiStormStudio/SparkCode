...
import { getAWSRegion, getDefaultVertexRegion, isEnvTruthy, getSparkEnv } from './envUtils.js';
...
export function buildMcpProperties(clients: MCPServerConnection[] = [], theme: ThemeName): Property[] {
  ...
  if (byState.needsAuth) parts.push(color('warning', theme)(`${byState.needsAuth} 个需要认证`));
...
export async function buildMemoryDiagnostics(): Promise<Diagnostic[]> {
  ...
  largeFiles.forEach(file => {
    ...
    diagnostics.push(`${displayPath} 文件过大，会影响性能（${formatNumber(file.content.length)} 字符 > ${formatNumber(MAX_MEMORY_CHARACTER_COUNT)}）`);
  });
  return diagnostics;
}
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(getSparkEnv("SKIP_BEDROCK_AUTH"))) {
      properties.push({
        value: '已跳过 AWS 认证'
      });
    }
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && getSparkEnv("CLIENT_CERT")) {
      properties.push({
        label: 'mTLS 客户端证书',
        value: getSparkEnv("CLIENT_CERT")
      });
    }
    if (mtlsConfig.key && getSparkEnv("CLIENT_KEY")) {
      properties.push({
        label: 'mTLS 客户端密钥',
        value: getSparkEnv("CLIENT_KEY")
      });
    }
  }
  return properties;
