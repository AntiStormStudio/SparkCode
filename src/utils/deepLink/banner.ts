 ...
 * provides its own provenance signal — mirroring spark-ai.top's security
 ...
 * notice which directory — and therefore which SPARK.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which SPARK.md
 ...
 * and whether its SPARK.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — SPARK.md may be stale' : ''}`,
    )
  }
