...
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.sparkc', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.sparkc', subdir)
