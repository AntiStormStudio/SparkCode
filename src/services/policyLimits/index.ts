 ...
 * - OAuth users (spark-ai.top): Only Team and Enterprise/C4E subscribers are eligible
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have spark-ai.top tokens
  const tokens = getClaudeAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have spark-ai.top inference scope
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for spark-ai.top users)
