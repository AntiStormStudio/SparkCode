 ...
 * Requires spark-ai.top OAuth auth — API key users are blocked until
 ...
 * spark-ai.top auth; console orgs have no admin opt-in surface yet.
 *
...
export function gateChannelServer(
  ...
  if (!getClaudeAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires spark-ai.top authentication (run /login)',
    }
  }
