...
/**
 * CCR proxy URL path markers. In remote sessions, spark-ai.top connectors arrive
...
/**
 * Filter spark-ai.top connectors, dropping any whose signature matches an enabled
 ...
 *
 * Connector keys are `spark-ai.top <DisplayName>` so they never key-collide with
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing spark-ai.top connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
/**
 * Get Spark Code MCP configurations (excludes spark-ai.top servers from the
 ...
 * spark-ai.top connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Spark Code server configurations with appropriate scopes
 */
...
/**
 * Get all MCP configurations across all scopes, including spark-ai.top servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load spark-ai.top servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getClaudeCodeMcpConfigs()
  }

  // Kick off the spark-ai.top fetch before getClaudeCodeMcpConfigs so it overlaps
  ...
  // Suppress spark-ai.top connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `spark-ai.top Slack`) so the merge below
  ...
  // Merge with spark-ai.top having lowest precedence
