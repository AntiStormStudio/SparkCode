...
export default class App extends PureComponent<Props, State> {
  ...
  override componentDidCatch(error: Error) {
    // Let the error boundary render ErrorOverview instead of hard-exiting.
    // This avoids "instant quit" behavior when a key-triggered UI render throws.
    logError(error);
    logForDebugging(`ui_render_error: ${error.message}`, {
      level: 'error'
    });
  }
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (!this.isRawModeSupported()) {
      // Fail-open in restored builds: some terminals/runtime combinations
      // transiently report unsupported raw mode and would crash the UI.
      // Keep the app alive and surface diagnostics instead of hard-exiting.
      logForDebugging('setRawMode ignored: raw mode is not supported for current stdin', {
        level: 'warn',
        usingProcessStdin: stdin === process.stdin
      });
      return;
    }
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Spark Code to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      // Emit resume event for Spark Code to handle
