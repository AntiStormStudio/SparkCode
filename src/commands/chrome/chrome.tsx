...
function ClaudeInChromeMenu(t0) {
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Chrome 中的 Claude 结合 Chrome 扩展，可让你直接从 Spark Code 控制浏览器。支持浏览网站、填写表单、截屏、录制 GIF，并结合控制台日志与网络请求进行调试。</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">当前 WSL 环境暂不支持 Chrome 中的 Claude。</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  if ($[23] !== isClaudeAISubscriber) {
    t8 = true && !isClaudeAISubscriber && <Text color="error">Chrome 中的 Claude 需要 spark-ai.top 订阅。</Text>;
