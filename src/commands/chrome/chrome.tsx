...
const CHROME_EXTENSION_URL = 'https://spark-ai.top/chrome';
...
function ClaudeInChromeMenu(t0) {
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Chrome 中的 Spark 结合 Chrome 扩展，可让你直接从 Spark Code 控制浏览器。支持浏览网站、填写表单、截屏、录制 GIF，并结合控制台日志与网络请求进行调试。</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">当前 WSL 环境暂不支持 Chrome 中的 Spark。</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  if ($[23] !== isClaudeAISubscriber) {
    t8 = true && !isClaudeAISubscriber && <Text color="error">Chrome 中的 Spark 需要 spark-ai.top 订阅。</Text>;
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>状态：{" "}{isConnected ? <Text color="success">已启用</Text> : <Text color="inactive">未启用</Text>}</Text><Text>扩展：{" "}{isExtensionInstalled ? <Text color="success">已安装</Text> : <Text color="warning">未检测到</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">安装完成后，请选择 {"\"重新连接扩展\""} 以建立连接。</Text>}<Text><Text dimColor={true}>用法： </Text><Text>claude --chrome</Text><Text dimColor={true}> 或 </Text><Text>claude --no-chrome</Text></Text><Text dimColor={true}>站点级权限继承自 Chrome 扩展。你可以在 Chrome 扩展设置中管理权限，控制 Claude 可浏览、点击和输入的网站范围。</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
