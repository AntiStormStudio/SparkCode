...
export function DiscoverPlugins({
      ...
      try {
        ...
        if (targetPlugin) {
          ...
          if (foundPlugin) {
            ...
            setError(`未在任何插件市场中找到插件“${targetPlugin}”`);
          }
        }
      } catch (err) {
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      const message = `✓ 已安装 ${successCount_0} 个插件。运行 /reload-plugins 激活。`;
      setResult(message);
    } else if (successCount_0 === 0) {
      setError(`安装失败：${formatFailureDetails(newFailedPlugins, true)}`);
    } else {
      const message_0 = `✓ 已安装 ${successCount_0}/${successCount_0 + failureCount} 个插件。失败：${formatFailureDetails(newFailedPlugins, false)}。运行 /reload-plugins 激活已成功安装的插件。`;
      setResult(message_0);
    }
    if (successCount_0 > 0) {
  ...
  if (typeof viewState === 'object' && viewState.type === 'plugin-options') {
    ...
    return <PluginOptionsFlow plugin={plugin_4} pluginId={pluginId_0} onDone={(outcome, detail) => {
      switch (outcome) {
        case 'configured':
          finish(`✓ 已安装并配置 ${plugin_4.name}。运行 /reload-plugins 应用更改。`);
          break;
        case 'skipped':
          finish(`✓ 已安装 ${plugin_4.name}。运行 /reload-plugins 应用更改。`);
          break;
        case 'error':
  ...
  return <Box flexDirection="column">
      ...
      {filteredPlugins.length === 0 && searchQuery && <Box marginBottom={1}>
          <Text dimColor>没有匹配“{searchQuery}”的插件</Text>
        </Box>}
    ...
    case "git-not-installed":
      {
        ...
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>安装插件市场需要 Git。</Text><Text dimColor={true}>请安装 Git 后重启 Spark Code。</Text></>;
          $[0] = t1;
        } else {
          t1 = $[0];
        }
    ...
    case "all-blocked-by-policy":
      {
        ...
        if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>你的组织策略不允许使用任何外部插件市场。</Text><Text dimColor={true}>请联系管理员。</Text></>;
          $[1] = t1;
        } else {
          t1 = $[1];
        }
    ...
    case "policy-restricts-sources":
      {
        let t1;
        if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>你的组织限制了可添加的插件市场来源。</Text><Text dimColor={true}>切到“插件市场”标签查看允许的来源。</Text></>;
          $[2] = t1;
        } else {
          t1 = $[2];
        }
    ...
    case "all-marketplaces-failed":
      {
        ...
        if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>无法加载插件市场数据。</Text><Text dimColor={true}>请检查网络连接。</Text></>;
          $[3] = t1;
        } else {
          t1 = $[3];
        }
    ...
    case "all-plugins-installed":
      {
        ...
        if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>所有可用插件都已安装。</Text><Text dimColor={true}>稍后再看看新插件，或添加更多插件市场。</Text></>;
          $[4] = t1;
        } else {
          t1 = $[4];
        }
