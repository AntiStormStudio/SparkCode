...
export function PluginSettings(t0) {
  ...
  if (viewState.type === "help") {
    ...
    if ($[28] === Symbol.for("react.memo_cache_sentinel")) {
      t16 = <Box flexDirection="column"><Text bold={true}>插件命令用法：</Text><Text> </Text><Text dimColor={true}>安装：</Text><Text> /plugin install - 浏览并安装插件</Text><Text>{" "}{"/plugin install <marketplace> - 从指定插件市场安装"}</Text><Text>{" /plugin install <plugin> - 安装指定插件"}</Text><Text>{" "}{"/plugin install <plugin>@<market> - 从插件市场安装插件"}</Text><Text> </Text><Text dimColor={true}>管理：</Text><Text> /plugin manage - 管理已安装插件</Text><Text>{" /plugin enable <plugin> - 启用插件"}</Text><Text>{" /plugin disable <plugin> - 停用插件"}</Text><Text>{" /plugin uninstall <plugin> - 卸载插件"}</Text><Text> </Text><Text dimColor={true}>插件市场：</Text><Text> /plugin marketplace - 插件市场管理菜单</Text><Text> /plugin marketplace add - 添加插件市场</Text><Text>{" "}{"/plugin marketplace add <path/url> - 直接添加插件市场"}</Text><Text> /plugin marketplace update - 更新插件市场</Text><Text>{" "}{"/plugin marketplace update <name> - 更新指定插件市场"}</Text><Text> /plugin marketplace remove - 移除插件市场</Text><Text>{" "}{"/plugin marketplace remove <name> - 移除指定插件市场"}</Text><Text> /plugin marketplace list - 列出所有插件市场</Text><Text> </Text><Text dimColor={true}>验证：</Text><Text>{" "}{"/plugin validate <path> - 验证 manifest 文件或目录"}</Text><Text> </Text><Text dimColor={true}>其他：</Text><Text> /plugin - 主插件菜单</Text><Text> /plugin help - 显示此帮助</Text><Text> /plugins - /plugin 的别名</Text></Box>;
      $[28] = t16;
    } else {
      t16 = $[28];
    }
