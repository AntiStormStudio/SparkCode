...
export function CheckExistingSecretStep(t0) {
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Box marginBottom={1}><Text>你想要：</Text></Box>;
    $[17] = t10;
  } else {
    t10 = $[17];
  }
  ...
  let t14;
  if ($[26] !== t13) {
    t14 = <Box marginBottom={1}><Text>{t13}使用其他名称创建新的 secret</Text></Box>;
    $[26] = t13;
    $[27] = t14;
  } else {
    t14 = $[27];
  ...
  if ($[28] !== cursorOffset || $[29] !== onSecretNameChange || $[30] !== onSubmit || $[31] !== secretName || $[32] !== terminalSize || $[33] !== useExistingSecret) {
    t15 = !useExistingSecret && <><Box marginBottom={1}><Text>输入新的 secret 名称（字母、数字和下划线）：</Text></Box><TextInput value={secretName} onChange={onSecretNameChange} onSubmit={onSubmit} focus={true} placeholder="例如：CLAUDE_API_KEY" columns={terminalSize.columns} cursorOffset={cursorOffset} onChangeCursorOffset={setCursorOffset} showCursor={true} /></>;
    $[28] = cursorOffset;
