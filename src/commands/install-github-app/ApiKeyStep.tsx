...
export function ApiKeyStep(t0) {
  ...
  if ($[27] !== existingApiKey || $[28] !== selectedOption || $[29] !== theme) {
    t11 = existingApiKey && <Box marginBottom={1}><Text>{selectedOption === "existing" ? color("success", theme)("> ") : "  "}使用现有 Spark Code API key</Text></Box>;
    $[27] = existingApiKey;
    $[28] = selectedOption;
    $[29] = theme;
    $[30] = t11;
  ...
  if ($[31] !== onCreateOAuthToken || $[32] !== selectedOption || $[33] !== theme) {
    t12 = onCreateOAuthToken && <Box marginBottom={1}><Text>{selectedOption === "oauth" ? color("success", theme)("> ") : "  "}使用 Claude 订阅创建长期 token</Text></Box>;
