...
export function SuccessStep(t0) {
  ...
  let t3;
  if ($[1] !== skipWorkflow) {
    t3 = !skipWorkflow && <Text color="success">✓ GitHub Actions workflow 已创建</Text>;
    $[1] = skipWorkflow;
    $[2] = t3;
  } else {
    t3 = $[2];
  ...
  if ($[3] !== secretExists || $[4] !== useExistingSecret) {
    t4 = secretExists && useExistingSecret && <Box marginTop={1}><Text color="success">✓ 已使用现有的 ANTHROPIC_API_KEY secret</Text></Box>;
    $[3] = secretExists;
    $[4] = useExistingSecret;
    $[5] = t4;
  } else {
  ...
  let t7;
  if ($[11] !== skipWorkflow) {
    t7 = skipWorkflow ? <><Text>1. 如果还没安装，请先安装 Claude GitHub App</Text><Text>2. workflow 文件已保持不变</Text><Text>3. API key 已配置，可直接使用</Text></> : <><Text>1. 已创建预填好的 PR 页面</Text><Text>2. 如果还没安装，请先安装 Claude GitHub App</Text><Text>3. 合并 PR 后即可启用 Claude PR 辅助</Text></>;
    $[11] = skipWorkflow;
    $[12] = t7;
  } else {
    t7 = $[12];
