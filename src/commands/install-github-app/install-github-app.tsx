  ...
  const checkGitHubCLI = useCallback(async () => {
    ...
    if (ghVersionResult.exitCode !== 0) {
      warnings.push({
        ...
        message: 'GitHub CLI（gh）似乎未安装，或当前无法访问。',
        instructions: ['从 https://cli.github.com/ 安装 GitHub CLI', 'macOS：brew install gh', 'Windows：winget install --id GitHub.cli', 'Linux：查看安装说明 https://github.com/cli/cli#installation']
      });
    }
    ...
    if (authResult.exitCode !== 0) {
      warnings.push({
        ...
        message: 'GitHub CLI 似乎还没有完成认证。',
        instructions: ['运行：gh auth login', '按提示完成 GitHub 认证', '也可以用环境变量或其他方式配置认证']
      });
    } else {
    ...
    if (state.step === 'warnings') {
      ...
      if (permissionCheck.error === 'repository_not_found') {
        repoWarnings.push({
          ...
          instructions: [`检查仓库名称是否正确：${repoName_1}`, '确认你有此仓库的访问权限', '如果是私有仓库，请确认 GitHub token 拥有 “repo” scope', '可以用此命令添加 repo scope：gh auth refresh -h github.com -s repo,workflow']
        });
      } else if (!permissionCheck.hasAccess) {
        repoWarnings.push({
          title: '需要管理员权限',
          message: `你可能需要 ${repoName_1} 的管理员权限才能设置 GitHub Actions。`,
          instructions: ['仓库管理员可以安装 GitHub App 并设置 secrets', '如果设置失败，请让仓库管理员运行此命令', '也可以使用手动设置说明']
        });
      }
