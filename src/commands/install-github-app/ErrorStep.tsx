...
export function ErrorStep(t0) {
  ...
  if ($[5] !== errorInstructions) {
    t4 = errorInstructions && errorInstructions.length > 0 && <Box flexDirection="column" marginTop={1}><Text dimColor={true}>修复方式：</Text>{errorInstructions.map(_temp)}</Box>;
    $[5] = errorInstructions;
    $[6] = t4;
  } else {
    t4 = $[6];
  ...
  if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Box marginTop={1}><Text dimColor={true}>手动设置说明：{" "}<Text color="claude">{GITHUB_ACTION_SETUP_DOCS_URL}</Text></Text></Box>;
    $[7] = t5;
  } else {
    t5 = $[7];
  }
