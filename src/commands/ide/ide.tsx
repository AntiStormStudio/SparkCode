...
function IDEScreen(t0) {
  ...
  let t7;
  if ($[24] !== availableIDEs) {
    t7 = availableIDEs.length !== 0 && availableIDEs.some(_temp2) && <Box marginTop={1}><Text color="warning">注意：同一时间只能有一个 Spark Code 实例连接到 VS Code。</Text></Box>;
    $[24] = availableIDEs;
    $[25] = t7;
  } else {
    t7 = $[25];
  ...
  if ($[26] !== availableIDEs.length) {
    t8 = availableIDEs.length !== 0 && !isSupportedTerminal() && <Box marginTop={1}><Text dimColor={true}>提示：可以在 /config 中或通过 --ide 参数启用 IDE 自动连接。</Text></Box>;
  ...
  if ($[28] !== unavailableIDEs) {
    t9 = unavailableIDEs.length > 0 && <Box marginTop={1} flexDirection="column"><Text dimColor={true}>发现另外 {unavailableIDEs.length} 个正在运行的 IDE，但它们的工作区或项目目录与当前目录不匹配。</Text><Box marginTop={1} flexDirection="column">{unavailableIDEs.map(_temp3)}</Box></Box>;
    $[28] = unavailableIDEs;
    $[29] = t9;
  } else {
    t9 = $[29];
  ...
  if ($[36] !== onClose || $[37] !== t10) {
    t11 = <Dialog title="选择 IDE" subtitle="连接到 IDE 以使用集成开发功能。" onCancel={onClose} color="ide">{t10}</Dialog>;
    $[36] = onClose;
    $[37] = t10;
    $[38] = t11;
  } else {
...
export async function call(onDone: (result?: string, options?: {
  ...
  if (args?.trim() === 'open') {
    ...
    if (availableIDEs.length === 0) {
      onDone('未检测到安装了 Spark Code 扩展的 IDE。');
      return null;
    }
...
function IDECommandFlow({
  ...
  return <IDEScreen availableIDEs={availableIDEs} unavailableIDEs={unavailableIDEs} selectedIDE={currentIDE} onClose={() => onDone('已取消选择 IDE', {
    display: 'system'
