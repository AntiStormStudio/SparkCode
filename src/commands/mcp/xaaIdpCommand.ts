...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "Spark Code's client_id at the IdP")
    .option(
      '--client-secret',
    ...
    .action(options => {
      ...
      if (options.clientSecret && !secret) {
        return cliError(
          '错误：--client-secret 需要环境变量 MCP_XAA_IDP_CLIENT_SECRET',
        )
      }
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (options.idToken) {
        ...
        return cliOk(
          `已缓存 ${idp.issuer} 的 id_token（过期时间 ${new Date(expiresAt).toISOString()}）`,
        )
      }
      ...
      if (wasCached) {
        return cliOk(
          `已登录 ${idp.issuer}（缓存的 id_token 仍有效）。如需重新登录请使用 --force。`,
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Client secret：${hasSecret ? '（已存入钥匙串）' : '（未设置，仅 PKCE）'}\n`,
      )
      process.stdout.write(
        `登录状态：      ${hasIdToken ? '已登录（id_token 已缓存）' : "未登录，请运行 'claude mcp xaa login'"}\n`,
      )
      cliOk()
    })
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      if (error) {
        return cliError(`写入设置失败：${error.message}`)
      }
