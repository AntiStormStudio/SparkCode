    ...
    try {
      ...
      if (executionTarget === 'remote') {
        ...
        enqueuePendingNotification({
          value: [`Ultraplan approved — executing in Spark Code on the web. Follow along at: ${url}`, '', 'Results will land as a pull request when the remote session finishes. There is nothing to do here.'].join('\n'),
...
function buildLaunchMessage(disconnectedBridge?: boolean): string {
  ...
  return `${DIAMOND_OPEN} ultraplan\n${prefix}Starting Spark Code on the web…`;
}
function buildSessionReadyMessage(url: string): string {
  return `${DIAMOND_OPEN} ultraplan · 在网页端 Spark Code 查看进度 ${url}\n你可以继续工作，等 ${DIAMOND_OPEN} 填满后按 ↓ 查看结果`;
}
function buildAlreadyActiveMessage(url: string | undefined): string {
  return url ? `ultraplan：已在轮询。打开 ${url} 查看状态，或等待计划回到这里。` : 'ultraplan：正在启动，请等待会话开始。';
}
...
export async function launchUltraplan(opts: {
  ...
  if (!blurb && !seedPlan) {
    ...
    'Usage: /ultraplan \\<prompt\\>, or include "ultraplan" anywhere', 'in your prompt', '', 'Advanced multi-agent plan mode with our most powerful model', '(Opus). Runs in Spark Code on the web. When the plan is ready,', 'you can execute it in the web session or send it back here.', 'Terminal stays free while the remote plans.', 'Requires /login.', '', `Terms: ${CCR_TERMS_URL}`].join('\n');
  }
...
export default {
  type: 'local-jsx',
  name: 'ultraplan',
  description: `~10–30 min · Spark Code on the web drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
  argumentHint: '<prompt>',
