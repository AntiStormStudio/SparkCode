...
function Web({
        ...
        case 'gh_not_authenticated':
          {
            ...
            onDone(result.status === 'gh_not_installed' ? `未找到 GitHub CLI。请通过 https://cli.github.com/ 安装，然后运行 \`gh auth login\`，或在网页端连接 GitHub：${url}` : `GitHub CLI 尚未认证。请运行 \`gh auth login\` 后重试，或在网页端连接 GitHub：${url}`);
            return;
          }
        case 'has_gh_token':
          setStep({
  ...
  const handleConfirm = async (token: RedactedGithubToken) => {
    ...
    onDone(`已以 ${result.result.github_username} 身份连接。已打开 ${url}`);
  };
  ...
  return <Dialog title="将网页端 Claude 连接到 GitHub？" onCancel={handleCancel} hideInputGuide>
      <Box flexDirection="column">
        <Text>
          网页端 Claude 需要连接你的 GitHub 账户，才能代表你克隆和推送代码。
        </Text>
        <Text dimColor>
          将使用你的本地凭据向 GitHub 认证
        </Text>
      </Box>
      <Select options={[{
      label: '继续',
