...
export function shouldOfferTerminalSetup(): boolean {
  ...
  // keyboard protocol, which Spark Code already parses. No setup needed for
  // these terminals.
...
async function enableOptionAsMetaForProfile(profileName: string): Promise<boolean> {
  ...
  if (addCode !== 0) {
    ...
    if (setCode !== 0) {
      logError(new Error(`为 Terminal.app 配置 ${profileName} 启用 Option 作为 Meta 键失败`));
      return false;
    }
  }
  return true;
...
async function disableAudioBellForProfile(profileName: string): Promise<boolean> {
  ...
  if (addCode !== 0) {
    ...
    if (setCode !== 0) {
      logError(new Error(`为 Terminal.app 配置 ${profileName} 禁用提示音失败`));
      return false;
    }
  }
  return true;
...
async function enableOptionAsMetaForTerminal(theme: ThemeName): Promise<string> {
  try {
    ...
    if (!backupPath) {
      throw new Error('备份 Terminal.app 偏好设置失败，已停止');
    }
    ...
    if (readCode !== 0 || !defaultProfile.trim()) {
      throw new Error('读取默认 Terminal.app 配置失败');
    }
    const {
    ...
    if (startupCode !== 0 || !startupProfile.trim()) {
      throw new Error('读取启动 Terminal.app 配置失败');
    }
    ...
    if (!wasAnyProfileUpdated) {
      throw new Error('未能为任何 Terminal.app 配置启用 Option 作为 Meta 键或禁用提示音');
    }
