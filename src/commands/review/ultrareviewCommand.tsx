...
export const call: LocalJSXCommandCall = async (onDone, context, args) => {
  ...
  if (gate.kind === 'not-enabled') {
    onDone('Free ultrareviews used. Enable Extra Usage at https://spark-ai.top/settings/billing to continue.', {
      display: 'system'
    });
    return null;
  }
  if (gate.kind === 'low-balance') {
    onDone(`Balance too low to launch ultrareview ($${gate.available.toFixed(2)} available, $10 minimum). Top up at https://spark-ai.top/settings/billing`, {
      display: 'system'
    });
    return null;
  }
