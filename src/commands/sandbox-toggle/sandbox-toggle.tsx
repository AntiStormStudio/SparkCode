...
export async function call(onDone: (result?: string) => void, _context: unknown, args?: string): Promise<React.ReactNode | null> {
  ...
  if (!SandboxManager.isSupportedPlatform()) {
    ...
    const errorMessage = platform === 'wsl' ? '错误：沙箱需要 WSL2，不支持 WSL1。' : '错误：沙箱目前只支持 macOS、Linux 和 WSL2。';
  ...
  if (!SandboxManager.isPlatformInEnabledList()) {
    const message = color('error', themeName)(`错误：enabledPlatforms 设置已禁用当前平台（${platform}）的沙箱。`);
    onDone(message);
    return null;
  }
  ...
  if (SandboxManager.areSandboxSettingsLockedByPolicy()) {
    const message = color('error', themeName)('错误：沙箱设置已被更高优先级的配置覆盖，无法在本地修改。');
    onDone(message);
    return null;
  }
