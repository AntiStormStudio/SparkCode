...
function getInstallationPath(): string {
  ...
  if (isWindows) {
    ...
    const windowsPath = join(homeDir, '.local', 'bin', 'sparkc.exe');
    // Replace forward slashes with backslashes for Windows display
    return windowsPath.replace(/\//g, '\\');
  }
  return '~/.local/bin/sparkc';
}
...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('无法安装：另一个进程正在安装 SPARK-Code，请稍后再试。');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'SPARK-Code 安装完成', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'SPARK-Code 安装失败', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing SPARK-Code native build {state.version}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              SPARK-Code successfully installed!
            </Text>
          </Box>
      ...
      {state.type === 'error' && <Box flexDirection="column" gap={1}>
          ...
          <Box marginTop={1}>
            <Text dimColor>可尝试使用 --force 覆盖检查</Text>
          </Box>
        </Box>}
    </Box>;
}
...
export const install = {
  ...
  name: 'install',
  description: '安装 SPARK-Code 原生构建',
