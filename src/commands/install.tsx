...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing SPARK-Code. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'SPARK-Code installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'SPARK-Code installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
...
export const install = {
  ...
  name: 'install',
  description: 'Install SPARK-Code native build',
