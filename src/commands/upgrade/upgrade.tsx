...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext): Promise<React.ReactNode | null> {
  try {
    ...
    const url = 'https://spark-ai.top/upgrade/max';
    await openBrowser(url);
    return <Login startingMessage={'正在根据 /upgrade 开始新的登录。按 Ctrl-C 退出并使用现有账号。'} onDone={success => {
      context.onChangeAPIKey();
      onDone(success ? '登录成功' : '登录已中断');
    }} />;
  } catch (error) {
    logError(error as Error);
    setTimeout(onDone, 0, '无法打开浏览器。请访问 https://spark-ai.top/upgrade/max 升级。');
  }
  return null;
}
