...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        '尚未安装插件。使用 `sparkc plugin install` 安装插件。',
      )
    }
  }
...
export async function marketplaceAddHandler(
  ...
  try {
    ...
    if (scope !== 'user' && scope !== 'project' && scope !== 'local') {
      cliError(
        `${figures.cross} 作用域 '${scope}' 无效。请使用：user、project 或 local`,
      )
    }
    ...
    if (options.sparse && options.sparse.length > 0) {
      if (
        ...
        cliError(
          `${figures.cross} --sparse 只支持 github 和 git 类型的 marketplace 来源（当前：${marketplaceSource.source}）`,
        )
      }
    }
    ...
    cliOk(
      alreadyMaterialized
        ? `${figures.tick} Marketplace '${name}' 已在磁盘上，已写入 ${scope} 设置`
        : `${figures.tick} 已成功添加 marketplace：${name}（写入 ${scope} 设置）`,
    )
  } catch (error) {
...
export async function marketplaceListHandler(options: {
  ...
  try {
    ...
    handleMarketplaceError(error, '列出 marketplace')
  }
}
...
export async function pluginInstallHandler(
  ...
  if (
    ...
    cliError(
      `作用域无效：${scope}。必须是以下之一：${VALID_INSTALLABLE_SCOPES.join(', ')}。`,
    )
  }
...
export async function pluginEnableHandler(
  ...
  if (options.scope) {
    if (
      ...
      cliError(
        `作用域 "${options.scope}" 无效。有效作用域：${VALID_INSTALLABLE_SCOPES.join(', ')}`,
      )
    }
...
export async function pluginDisableHandler(
  ...
  if (!options.all && !plugin) {
    cliError('请指定插件名，或使用 --all 停用所有插件')
  }
...
export async function pluginUpdateHandler(
  ...
  if (options.scope) {
    if (
      ...
      cliError(
        `作用域 "${options.scope}" 无效。有效作用域：${VALID_UPDATE_SCOPES.join(', ')}`,
      )
    }
