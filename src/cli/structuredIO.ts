...
export class StructuredIO {
   ...
   * Used by the bridge to feed permission responses from spark-ai.top into the
   * SDK permission flow.
   *
   ...
   * stale permission prompt on spark-ai.top when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt on spark-ai.top.
        if (
